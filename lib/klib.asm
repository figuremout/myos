...
putString:
    ...
    mov eax, [ebp+8]                             ; get param linear_addr
    mov [ebp-4], eax
...
putChar:
    ...
    mov ecx, [ebp+8]                            ; get param char ascii
    cmp cl, 0x0d                                ; if CR
    jnz .if_0a
    ; char is CR
    mov bl, 80
...
.put_other:                                     ; display normal char
    ...
    mov [0xb8000+ebx], cl                    ; fill in ascii
    mov byte [0xb8000+ebx+1], 0x07           ; fill in attr
...
.roll_screen:
    ...
    cld
    mov edi,0xb8000                          ; es:di point to 1st row start
    mov esi,0xb80a0                          ; ds:si point to 2nd row start
    ...
    mov ecx, 80
.cls:
    mov word [0xb8000+ebx], 0x0720           ; 0x0720 bg=black fg=white char=space
    add bx,2
    loop .cls
